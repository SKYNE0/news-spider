...
class huxiu(object):
    ...
    def get_news(self, url):
        ...
        try:
            ...
            news['title_only'] = selector.xpath('/html/head/title/text()')[0]
            news['author'] = u'虎嗅网'
            # selector.xpath('//div[3]/div[1]/div[2]/a[1]/text()')[0].strip()
            news['summary'] = str(selector.xpath('//div[@class="article-content-wrap"]//p/text()'))[1:200:1].strip("', '").replace ("', '", '').replace('\\xa0', '')
            news['title'] = news['title_only'] + '\n' + news['summary'] + '........\n\n点击查看全部'
